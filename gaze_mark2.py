...
class Eye:
    def __init__(self, name, landmarks, pointArray):
        ...
        self.left_point = (
            self.landmarks.part(self.pointArray[0]).x,
            self.landmarks.part(self.pointArray[0]).y,
        )
        self.right_point = (
            self.landmarks.part(self.pointArray[1]).x,
            self.landmarks.part(self.pointArray[1]).y,
        )
        self.center_top = midPoint(
            self.landmarks.part(self.pointArray[2]),
            self.landmarks.part(self.pointArray[3]),
        )
        self.center_bottom = midPoint(
            self.landmarks.part(self.pointArray[4]),
            self.landmarks.part(self.pointArray[5]),
        )

    def drawLines(self, frame):
        hor_line = cv2.line(
            frame, self.left_point, self.right_point, (255, 255, 255), 2
        )
        ver_line = cv2.line(
            frame, self.center_top, self.center_bottom, (255, 255, 255), 2
        )
    ...
    def get_gaze_ratio(self, frame, gray):
        left_eye_region = np.array(
            [
                ...
                (
                    self.landmarks.part(self.pointArray[2]).x,
                    self.landmarks.part(self.pointArray[2]).y,
                ),
                (
                    self.landmarks.part(self.pointArray[3]).x,
                    self.landmarks.part(self.pointArray[3]).y,
                ),
                (
                    self.landmarks.part(self.pointArray[1]).x,
                    self.landmarks.part(self.pointArray[1]).y,
                ...
                (
                    self.landmarks.part(self.pointArray[5]).x,
                    self.landmarks.part(self.pointArray[5]).y,
                ),
                (
                    self.landmarks.part(self.pointArray[4]).x,
                    self.landmarks.part(self.pointArray[4]).y,
                ),
            ],
            np.int32,
        )
