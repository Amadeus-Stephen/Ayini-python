...
class Eye_base:
    def __init__(self, side,points,thresh, landmarks ):
        ...
        self.left_point    = (self.landmarks.part(self.points[0]).x , self.landmarks.part(self.points[0]).y)
        self.right_point   = (self.landmarks.part(self.points[1]).x , self.landmarks.part(self.points[1]).y)
        self.center_top    = self.mid_point(self.landmarks.part(self.points[2]), self.landmarks.part(self.points[3]))
        self.center_bottom = self.mid_point(self.landmarks.part(self.points[4]), self.landmarks.part(self.points[5]))
        ...
        if self.cap.isOpened():
            while True:
                ...
                for rect in rects:
                    ...
                    left_eye  = Eye_base("left",  [36, 37, 38, 40, 41, 40], thresh[:, 0:mid], landmarks)
                    right_eye = Eye_base("right", [42, 43, 44, 46, 47, 46], thresh[:,  mid:], landmarks)
